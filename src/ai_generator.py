...
async def generate_answer_with_rag(query: str, company_name: str, inn: str, doc_type: str = "other") -> str:
    ...
    if retriever:
        try:
            # Увеличен top_k до 15 для лучшего покрытия
...
2. Цитируй статьи и пункты ТОЧНО, как они указаны в контексте.
3. НЕ ПРИДУМЫВАЙ статьи или нормы, которых нет в контексте.
4. Если в контексте нет ответа, прямо скажи: "В предоставленных правовых актах нет информации по данному вопросу."
5. Не добавляй список источников в конце ответа.
6. Если упоминаешь статью, убедись, что она реально есть в контексте.
